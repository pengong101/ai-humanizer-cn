...
def generate_report(date_str):
    ...
    for category, keywords in KEYWORDS.items():
        ...
        for kw in keywords:
            ...
            items.extend(results[:3])  # 每个关键词取 3 条
    ...
    # 保存报告
    output_file = f"/root/.openclaw/workspace/radar-reports/radar-daily-{date_str}-baidu.md"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"# 毫米波雷达日报 - {date_str}\n\n")
        f.write(f"**生成时间:** {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
        f.write(f"**数据来源:** SearXNG (百度引擎)\n")
        ...
        for cat_id, items in report['categories'].items():
            f.write(f"## {category_names.get(cat_id, cat_id)}\n\n")
            if items:
