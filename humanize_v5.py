...
PROTECTED_PATTERNS = [
    (r'\d+\.?\d*%', 'DATA'),  # 百分比
    (r'\d+\.?\d*\s*(倍|年|月|日|时|分|秒|℃|km|m|s|kg|Hz|GHz|MHz)', 'DATA'),  # 单位
    (r'[A-Z][a-z]*(?:\s+[A-Z][a-z]+)*', 'PROPER'),  # 英文术语/人名
    (r'[""](?:[^""（）()]|[""][^"""])*[""]', 'QUOTE'),  # 引号内容
    (r'（[^（）]*）|\([^（）]*\)', 'BRACKET'),  # 括号内容
]
...
class AIHumanizerV5:
    ...
    def __init__(self, config: Optional[HumanizerConfig] = None):
        ...
        self._protected_ranges: List[Tuple[int, int, str]] = []
        self._stats = {}
    ...
    def _protect_content(self, text: str) -> Tuple[str, List]:
        ...
        for pattern, label in PROTECTED_PATTERNS:
            for m in re.finditer(pattern, result):
                ...
                # 避免重叠
                if not self._overlaps(ranges, start, end):
                    placeholder = f"__PROTECTED_{label}_{len(ranges)}__"
                    result = result[:start + offset] + placeholder + result[end + offset:]
                    ranges.append((start, end, m.group(), label, offset))
                    offset += len(placeholder) - (end - start)

        return result, ranges

    def _overlaps(self, ranges: List, start: int, end: int) -> bool:
        for r in ranges:
    ...
    def _restore_content(self, text: str) -> str:
        """恢复被保护的内容"""
        result = text
        for placeholder, original, label, offset in sorted(self._protected_ranges, key=lambda x: x[0], reverse=True):
...
请直接输出修改后的文章，不需要解释。"""
    ...
    def _call_minimax(self, prompt: str, original: str) -> str:
        ...
        try:
            ...
            resp = json_mod.loads(r.stdout)
            if "choices" in resp and len(resp["choices"]) > 0:
                return resp["choices"][0]["messages"][-1]["content"].strip()
        except Exception:
            pass
        return self._fallback_to_rule_based(original)
    ...
    def _inject_connectors(self, text: str, ratio: float) -> str:
        """注入口语化连接词"""
        sentences = re.split(r'([。！？；\n])', text)
        result = []
        for i, seg in enumerate(sentences):
            if seg and len(seg) > 5 and i % 3 == 0:
